...
def run_worker(worker_id: int, goal: str, name: str, model: str, angle: str) -> dict:
    """Run a single worker agent (used by both parallel and sequential)."""
    system_prompt = (
...
def benchmark_parallel():
    """Run all 5 workers in parallel via ThreadPoolExecutor."""
    print(f"\n{'='*60}")
...
def benchmark_sequential():
    """Run all 5 workers one at a time (Claude-like sub-agent pattern)."""
    print(f"\n{'='*60}")
